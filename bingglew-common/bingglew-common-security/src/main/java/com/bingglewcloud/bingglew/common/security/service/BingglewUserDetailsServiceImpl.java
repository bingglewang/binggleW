...
@Service
@AllArgsConstructor
public class BingglewUserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if (cache != null && cache.get(username) != null) {
			return (BingglewUser) cache.get(username).get();
		}
	...
	private UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new BingglewUser(user.getUserId(), user.getDeptId(), user.getUsername(), SecurityConstants.BCRYPT + user.getPassword(),
